...
#
# QEMU is no longer built here — ndt.sh runs the system qemu-system-x86_64
# (override with QEMU_BIN=...).
#
...
#
# NVMe namespaces are provided by the in-guest vnvme + null_blk stack
# (see initramfs/rootfs/init), so there is no per-NS disk image to
# materialise here any more.
