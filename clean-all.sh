...
for arg in "$@"; do
    case "$arg" in
        -h|--help)
            ...
            exit 0
            ;;
        *) echo "[clean-all] unknown arg: $arg" >&2; exit 2 ;;
    esac
done
...
NPS=$NDT/third_party/nvmet-pci-sw
if [[ -f "$NPS/Makefile" && -f "$NDT/build/linux/include/config/kernel.release" ]]; then
    echo "[clean-all] make -C ${NPS#$NDT/} clean"
    make -C "$NPS" KDIR="$NDT/build/linux" clean >/dev/null
elif [[ -f "$NPS/nvmet-pci-sw.ko" ]]; then
    # KDIR is gone (rm -rf build/ above) so make clean would fail.  Wipe
